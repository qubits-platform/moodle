...
/**
 * Extends table_sql to provide a table of qbassignment submissions
 *
 * @package   mod_qbassign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassign_grading_table extends table_sql implements renderable {
    /** @var qbassign $qbassignment */
    private $qbassignment = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the qbassignment class that is displaying this table
     *
     ...
     */
    public function __construct(qbassign $qbassignment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_qbassign_grading-' . $qbassignment->get_context()->id);

        $this->is_persistent(true);
        $this->qbassignment = $qbassignment;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/qbassign:grantextension',
                                                  $this->qbassignment->get_context());
        $this->hasgrade = $this->qbassignment->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/qbassign:viewblinddetails',
                $this->qbassignment->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_qbassign');

        $urlparams = array('action' => 'grading', 'id' => $qbassignment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/qbassign/view.php', $urlparams);
        ...
        $users = array_keys( $qbassignment->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['qbassignmentid1'] = (int)$this->qbassignment->get_instance()->id;
        $params['qbassignmentid2'] = (int)$this->qbassignment->get_instance()->id;
        $params['qbassignmentid3'] = (int)$this->qbassignment->get_instance()->id;
        $params['newstatus'] = qbassign_SUBMISSION_STATUS_NEW;

        // TODO Does not support custom user profile fields (MDL-70456).
        $userfieldsapi = \core_user\fields::for_identity($this->qbassignment->get_context(), false)->with_userpic();
        ...
        // For group qbassignments, there can be a grade with no submission.
        $from .= ' LEFT JOIN {qbassign_grades} g
                            ON g.qbassignment = :qbassignmentid2
                           AND u.id = g.userid
                           AND (g.attemptnumber = s.attemptnumber OR s.attemptnumber IS NULL) ';

        $from .= 'LEFT JOIN {qbassign_user_flags} uf
                         ON u.id = uf.userid
                        AND uf.qbassignment = :qbassignmentid3 ';

        if ($this->qbassignment->get_course()->relativedatesmode) {
            $params['courseid1'] = $this->qbassignment->get_course()->id;
            $from .= ' LEFT JOIN (
        ...
        $hasoverrides = $this->qbassignment->has_overrides();
        $inrelativedatesmode = $this->qbassignment->get_course()->relativedatesmode;

        if ($hasoverrides) {
            $params['qbassignmentid5'] = (int)$this->qbassignment->get_instance()->id;
            $params['qbassignmentid6'] = (int)$this->qbassignment->get_instance()->id;
            $params['qbassignmentid7'] = (int)$this->qbassignment->get_instance()->id;
            $params['qbassignmentid8'] = (int)$this->qbassignment->get_instance()->id;
            $params['qbassignmentid9'] = (int)$this->qbassignment->get_instance()->id;
            ...
            $from .= ' LEFT JOIN (
               SELECT merged.userid, min(merged.priority) priority FROM (
                  ...
                  ( SELECT uo.userid, 0 AS priority
                      FROM {qbassign_overrides} uo
                     WHERE uo.qbassignid = :qbassignmentid5
                  )
                  UNION
                  ( SELECT gm.userid, go.sortorder AS priority
                      FROM {qbassign_overrides} go
                      JOIN {groups} g ON g.id = go.groupid
                      JOIN {groups_members} gm ON gm.groupid = g.id
                     WHERE go.qbassignid = :qbassignmentid6
                  )
                ) merged
            ...
            JOIN (
              (SELECT 9999999 AS priority,
                      ...
                      a.cutoffdate
                 FROM {user} u
                 JOIN {qbassign} a ON a.id = :qbassignmentid7
                 '.$userwhere2.'
              )
              UNION
              (SELECT 0 AS priority,
                      ...
                      uo.cutoffdate
                 FROM {qbassign_overrides} uo
                WHERE uo.qbassignid = :qbassignmentid8
              )
              UNION
              (SELECT go.sortorder AS priority,
                 ...
                 JOIN {groups} g ON g.id = go.groupid
                 JOIN {groups_members} gm ON gm.groupid = g.id
                WHERE go.qbassignid = :qbassignmentid9
              )
            ...
            $fields .= 'a.cutoffdate ';
            $from .= 'JOIN {qbassign} a ON a.id = :qbassignmentid5 ';
        }

        if (!empty($this->qbassignment->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {qbassign_user_mapping} um
                             ON u.id = um.userid
                            AND um.qbassignment = :qbassignmentidblind ';
            $params['qbassignmentidblind'] = (int)$this->qbassignment->get_instance()->id;
        ...
        if ($this->qbassignment->is_any_submission_plugin_enabled()) {
            if ($filter == qbassign_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = qbassign_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == qbassign_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status <> :submitted) ';
                $params['submitted'] = qbassign_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == qbassign_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified >= g.timemodified OR g.timemodified IS NULL OR g.grade IS NULL';

                // qbassignment grade is set to the negative grade scale id when scales are used.
            ...
            } else if ($filter == qbassign_FILTER_GRANTED_EXTENSION) {
                $where .= ' AND uf.extensionduedate > 0 ';

            } else if (strpos($filter, qbassign_FILTER_SINGLE_USER) === 0) {
                $userfilter = (int) array_pop(explode('=', $filter));
                $where .= ' AND (u.id = :userid)';
                $params['userid'] = $userfilter;
            } else if ($filter == qbassign_FILTER_DRAFT) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :draft) ';
                $params['draft'] = qbassign_SUBMISSION_STATUS_DRAFT;
            }
        }

        if ($this->qbassignment->get_instance()->markingworkflow &&
            $this->qbassignment->get_instance()->markingallocation) {
            if (has_capability('mod/qbassign:manageallocations', $this->qbassignment->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('qbassign_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == qbassign_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->qbassignment->get_instance()->markingworkflow) {
            $workflowstates = $this->qbassignment->get_marking_workflow_states_for_current_user();
            if (!empty($workflowstates)) {
                $workflowfilter = get_user_preferences('qbassign_workflowfilter', '');
                if ($workflowfilter == qbassign_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('qbassign_user_flags', 'workflowstate', true, true).')';
        ...
        // User picture.
        if ($this->hasviewblind || !$this->qbassignment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'qbassign');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'qbassign');

        if ($hasoverrides || $inrelativedatesmode) {
            // Allowsubmissionsfromdate.
            $columns[] = 'allowsubmissionsfromdate';
            $headers[] = get_string('allowsubmissionsfromdate', 'qbassign');

            // Duedate.
            $columns[] = 'duedate';
            $headers[] = get_string('duedate', 'qbassign');

            // Cutoffdate.
            $columns[] = 'cutoffdate';
            $headers[] = get_string('cutoffdate', 'qbassign');
        }

        // Team submission columns.
        if ($qbassignment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'qbassign');
        }
        // Allocated marker.
        if ($this->qbassignment->get_instance()->markingworkflow &&
            $this->qbassignment->get_instance()->markingallocation &&
            has_capability('mod/qbassign:manageallocations', $this->qbassignment->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'qbassign');
        }
        // Grade.
        $columns[] = 'grade';
        $headers[] = get_string('gradenoun');
        if ($this->is_downloading()) {
            $gradetype = $this->qbassignment->get_instance()->grade;
            if ($gradetype > 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'qbassign');
            } else if ($gradetype < 0) {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'qbassign');
            }

            if ($this->qbassignment->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'qbassign');
            }
            // Add a column to show if this grade can be changed.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'qbassign');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        // Submission plugins.
        if ($qbassignment->is_any_submission_plugin_enabled()) {
            $columns[] = 'timesubmitted';
            $headers[] = get_string('lastmodifiedsubmission', 'qbassign');
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'qbassign');
        ...
        $this->gradinginfo = grade_get_grades($this->qbassignment->get_course()->id,
                                              'mod',
                                              'qbassign',
                                              $this->qbassignment->get_instance()->id,
                                              $users);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this qbassignment.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        if (empty($row->recordid)) {
            $row->recordid = $this->qbassignment->get_uniqueid_for_user($row->userid);
        }
        return get_string('hiddenuser', 'qbassign') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        $gradingdisabled = $this->qbassignment->grading_disabled($row->id, true, $this->gradinginfo);
        // The function in the qbassignment keeps a static cache of this list of states.
        $workflowstates = $this->qbassignment->get_marking_workflow_states_for_current_user();
        $workflowstate = $row->workflowstate;
        if (empty($workflowstate)) {
            $workflowstate = qbassign_MARKING_WORKFLOW_STATE_NOTMARKED;
        }
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'qbassign'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = qbassign_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'qbassign');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_enrolled_users($this->qbassignment->get_context(), 'mod/qbassign:grade', 0, 'u.*', $sort);
            $markerlist[0] = get_string('choosemarker', 'qbassign');
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->qbassignment->get_context());
        ...
        if ($this->is_downloading()) {
            if (isset($markers[$row->allocatedmarker])) {
                return fullname($markers[$row->allocatedmarker],
                        has_capability('moodle/site:viewfullnames', $this->qbassignment->get_context()));
            } else {
                return '';
            }
        }

        if ($this->quickgrading && has_capability('mod/qbassign:manageallocations', $this->qbassignment->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == qbassign_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == qbassign_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id' => -($this->qbassignment->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->qbassignment->get_instance()->grade >= 0) {
                ...
                $gradeitem = $this->qbassignment->get_grade_item();
                return format_float($grade, $gradeitem->get_decimals());
            } else {
                // This is a custom scale.
                $scale = $this->qbassignment->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->qbassignment->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            return $group->name;
        } else if ($this->qbassignment->get_instance()->preventsubmissionnotingroup) {
            $usergroups = $this->qbassignment->get_all_groups($row->id);
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'qbassign');
            } else {
                return get_string('noteamgrader', 'qbassign');
            }
        }
        return get_string('defaultteam', 'qbassign');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->qbassignment->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->qbassignment->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $fullname = $this->output->action_link($link, $this->qbassignment->fullname($row));
        } else {
            $fullname = $this->qbassignment->fullname($row);
        }

        if (!$this->qbassignment->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'qbassign', $this->qbassignment->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this qbassignment.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        if ($this->qbassignment->get_instance()->grade > 0) {
            $gradeitem = $this->qbassignment->get_grade_item();
            return format_float($this->qbassignment->get_instance()->grade, $gradeitem->get_decimals());
        } else {
            return '';
        }
    }
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->qbassignment->get_instance($row->userid);
        ...
        if ($this->qbassignment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'qbassign'),
                                           array('class' => 'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != qbassign_SUBMISSION_STATUS_NEW) {
                ...
                && $status != qbassign_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'qbassign');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    if ($row->timemarked < $row->timesubmitted) {
                        $o .= $this->output->container(get_string('gradedfollowupsubmit', 'qbassign'), 'gradingreminder');
                    } else {
                        $o .= $this->output->container(get_string('graded', 'qbassign'), 'submissiongraded');
                    }
                } else if (!$timesubmitted || $status == qbassign_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'qbassign', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'qbassign', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'qbassign');
        }
        ...
        $submission = $row;
        $flags = $row;
        if ($this->qbassignment->get_instance()->teamsubmission) {
        ...
        $submissionsopen = $this->qbassignment->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        $caneditsubmission = $this->qbassignment->can_edit_submission($row->id, $USER->id);

        // Hide for offline qbassignments.
        if ($this->qbassignment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == qbassign_SUBMISSION_STATUS_DRAFT ||
                    !$this->qbassignment->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'qbassign');
                    ...
                    $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                                       ...
                                       'page' => $this->currpage);
                    $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'qbassign');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
                $description = get_string('editsubmission', 'qbassign');
            ...
            if ($USER->id != $row->id &&
                ...
                $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
                $description = get_string('removesubmission', 'qbassign');
        ...
        if (($this->qbassignment->get_instance()->duedate ||
                $this->qbassignment->get_instance()->cutoffdate) &&
             ...
             $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
             $description = get_string('grantextension', 'qbassign');
        ...
        if ($row->status == qbassign_SUBMISSION_STATUS_SUBMITTED &&
                $this->qbassignment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                               'userid' => $row->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'qbassign');
        ...
        if ($row->status == qbassign_SUBMISSION_STATUS_DRAFT &&
                $this->qbassignment->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
            $description = get_string('submitforgrading', 'qbassign');
        ...
        $ismanual = $this->qbassignment->get_instance()->attemptreopenmethod == qbassign_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != qbassign_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->qbassignment->get_instance()->maxattempts == qbassign_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->qbassignment->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->qbassignment->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/qbassign/view.php', $urlparams);
            $description = get_string('addattempt', 'qbassign');
     ...
     *
     * @param qbassign_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(qbassign_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('qbassign')), 'qbassign');
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'qbassignsubmission') {
                if ($this->qbassignment->get_instance()->teamsubmission) {
                    ...
                    $this->get_group_and_submission($row->id, $group, $submission, -1);
                    if ($submission) {
                        if ($submission->status == qbassign_SUBMISSION_STATUS_REOPENED) {
                    ...
                    if ($row->status == qbassign_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->qbassignment->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->qbassignment = $this->qbassignment->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->qbassignment = $this->qbassignment->get_instance()->id;
     ...
     *
     * @return string the qbassignment name
     */
    public function get_qbassignment_name() {
        return $this->qbassignment->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->qbassignment->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->qbassignment->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->qbassignment->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this qbassignment accept submissions
     */
    public function submissions_enabled() {
        return $this->qbassignment->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->qbassignment->get_course_context();
    ...
    public function get_sort_columns() {
        ...
        if (empty($qbassignment->blindmarking)) {
